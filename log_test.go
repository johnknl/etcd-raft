...
func TestFindConflictByTerm(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run("", func(t *testing.T) {
			...
			st.ApplySnapshot(pb.NewEmptySnapshot().SetMetadata(pb.NewEmptySnapshotMetadata().SetTermPtr(tt.ents[0].GetTerm()).SetIndexPtr(tt.ents[0].GetIndex())))
...
func TestCompactionSideEffects(t *testing.T) {
	...
	raftLog.append(pb.NewEntryRef(raftLog.lastIndex()+1, raftLog.lastIndex()+1))
...
func TestHasNextCommittedEnts(t *testing.T) {
	snap := pb.NewEmptySnapshot().SetMetadata(pb.NewEmptySnapshotMetadata().SetTermPtr(uint64(1)).SetIndex(uint64(3)))
...
func TestStableToWithSnap(t *testing.T) {
	...
	for i, tt := range tests {
		t.Run(fmt.Sprint(i), func(t *testing.T) {
			s := NewMemoryStorage()
			require.NoError(t, s.ApplySnapshot(pb.NewEmptySnapshot().SetMetadata(pb.NewEmptySnapshotMetadata().SetTermPtr(snapt).SetIndexPtr(snapi))))
...
func TestLogRestore(t *testing.T) {
	...
	term := uint64(1000)
	storage := NewMemoryStorage()
	storage.ApplySnapshot(pb.NewEmptySnapshot().SetMetadata(pb.NewEmptySnapshotMetadata().SetTermPtr(term).SetIndexPtr(index)))
	raftLog := newLog(storage, raftLogger)
...
func TestIsOutOfBounds(t *testing.T) {
	...
	num := uint64(100)
	storage := NewMemoryStorage()
	storage.ApplySnapshot(pb.NewEmptySnapshot().SetMetadata(pb.NewEmptySnapshotMetadata().SetIndexPtr(offset)))
...
func TestTerm(t *testing.T) {
	...
	storage := NewMemoryStorage()
	storage.ApplySnapshot(pb.NewEmptySnapshot().SetMetadata(pb.NewEmptySnapshotMetadata().SetTermPtr(uint64(1)).SetIndexPtr(offset)))
	l := newLog(storage, raftLogger)
...
func TestTermWithUnstableSnapshot(t *testing.T) {
	...
	storage := NewMemoryStorage()
	storage.ApplySnapshot(pb.NewEmptySnapshot().SetMetadata(pb.NewEmptySnapshotMetadata().SetTermPtr(uint64(1)).SetIndexPtr(storagesnapi)))
	l := newLog(storage, raftLogger)
	l.restore(pb.NewEmptySnapshot().SetMetadata(pb.NewEmptySnapshotMetadata().SetTermPtr(uint64(1)).SetIndexPtr(unstablesnapi)))
...
func TestSlice(t *testing.T) {
	...
	last := offset + num
	half := offset + num/2
	halfe := pb.NewEmptyEntry().SetType(pb.EntryNormal).SetTermPtr(half).SetIndex(half)
	...
	storage := NewMemoryStorage()
	require.NoError(t, storage.ApplySnapshot(pb.NewEmptySnapshot().SetMetadata(pb.NewEmptySnapshotMetadata().SetIndexPtr(offset))))
...
func (i index) terms(terms ...uint64) []*pb.Entry {
	...
	for _, term := range terms {
		entries = append(entries, pb.NewEntryRef(term, index))
		index++
	}
	return entries
}
...
func (i index) termRange(from, to uint64) []*pb.Entry {
	...
	for term := from; term < to; term++ {
		entries = append(entries, pb.NewEmptyEntry().SetType(pb.EntryNormal).SetTermPtr(term).SetIndexPtr(index))
		index++
	}
	return entries
}
